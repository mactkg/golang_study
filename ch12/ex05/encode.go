...
func MarshalJson(v interface{}) ([]byte, error) {
	var buf bytes.Buffer
	if err := encodeJson(&buf, reflect.ValueOf(v), 0); err != nil {
		return nil, err
	}
	return buf.Bytes(), nil
}
...
func encodeJson(buf *bytes.Buffer, v reflect.Value, indent int) error {
	switch v.Kind() {
	case reflect.Invalid:
	...
	case reflect.Ptr:
		return encodeJson(buf, v.Elem(), indent)
	...
	case reflect.Array, reflect.Slice: // (value ...)
		...
		for i := 0; i < v.Len(); i++ {
			...
			if err := encodeJson(buf, v.Index(i), indent); err != nil {
				return err
			}
	...
	case reflect.Struct: // ((name value) ...)
		...
		for i := 0; i < v.NumField(); i++ {
			...
			fmt.Fprintf(buf, `"%s": `, v.Type().Field(i).Name)
			if err := encodeJson(buf, v.Field(i), indent); err != nil {
				return err
			}
	...
	case reflect.Map: // ((key value) ...)
		...
		for i, key := range v.MapKeys() {
			...
			if err := encodeJson(buf, key, indent); err != nil {
				return err
			}
			buf.WriteString(": ")
			if err := encodeJson(buf, v.MapIndex(key), indent); err != nil {
				return err
			}
